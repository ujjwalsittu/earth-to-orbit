...
const User_1 = __importStar(require("../apps/api/src/models/User"));
const Organization_1 = __importStar(require("../apps/api/src/models/Organization"));
const Category_1 = __importDefault(require("../apps/api/src/models/Category"));
const Site_1 = __importDefault(require("../apps/api/src/models/Site"));
const Lab_1 = __importDefault(require("../apps/api/src/models/Lab"));
const Component_1 = __importDefault(require("../apps/api/src/models/Component"));
const Staff_1 = __importDefault(require("../apps/api/src/models/Staff"));
const MONGODB_URI = process.env.MONGODB_URI || "mongodb://localhost:27017/earth-to-orbit";
/**
...
const seedDatabase = async () => {
    try {
        ...
        await Promise.all([
            ...
            Site_1.default.deleteMany({}),
            Lab_1.default.deleteMany({}),
            Component_1.default.deleteMany({}),
            Staff_1.default.deleteMany({}),
        ]);
        console.log("✅ Existing data cleared\n");
        // Create Platform Admin
        console.log("👤 Creating platform admin...");
        const platformAdmin = await User_1.default.create({
            email: process.env.DEMO_PLATFORM_ADMIN_EMAIL || "admin@earth-to-orbit.com",
            password: process.env.DEMO_PLATFORM_ADMIN_PASSWORD || "Admin@123456",
            firstName: "Platform",
            lastName: "Admin",
            role: User_1.UserRole.PLATFORM_ADMIN,
        });
        ...
        const sites = await Site_1.default.insertMany([
            {
        ...
        const labs = await Lab_1.default.insertMany([
            {
            ...
            },
            {
                ...
                category: categories[0]._id,
                site: sites[1]._id,
                description: "Electromagnetic Interference and Compatibility testing facility. Ensures your satellite components meet international EMC standards and will not interfere with other systems.",
                specifications: new Map([
            ...
            },
            {
                ...
                category: categories[0]._id,
                site: sites[1]._id,
                description: "Pyroshock and mechanical shock testing facility. Simulates the intense shock pulses experienced during stage separation and other pyrotechnic events.",
                specifications: new Map([
        ...
        const components = await Component_1.default.insertMany([
            {
                name: "Star Tracker",
            ...
            },
            {
                ...
                site: sites[1]._id,
                description: "S-Band transceiver for telemetry, tracking, and command. Compatible with major ground station networks worldwide.",
                specifications: new Map([
            ...
            },
            {
                ...
                category: categories[3]._id,
                site: sites[0]._id,
                description: "High-capacity lithium-ion battery for satellite power storage. Optimized for deep discharge cycles and long mission life.",
                specifications: new Map([
                    ["Capacity", "40 Ah"],
            ...
            },
            {
                ...
                category: categories[2]._id,
                site: sites[1]._id,
                description: "Space-qualified GPS receiver for orbit determination and time synchronization.",
                specifications: new Map([
        ...
        const staff = await Staff_1.default.insertMany([
            {
                firstName: "Rajesh",
                lastName: "Kumar",
        ...
        const demoOrg = await Organization_1.default.create({
            name: "SpaceTech Innovations",
            legalName: "SpaceTech Innovations Private Limited",
            type: Organization_1.OrganizationType.STARTUP,
        ...
        const orgAdmin = await User_1.default.create({
        ...
        const orgMember = await User_1.default.create({
            ...
            role: User_1.UserRole.ORG_MEMBER,
            organization: demoOrg._id,
        });
        ...
        console.log(`   Platform Admin: ${platformAdmin.email} / ${process.env.DEMO_PLATFORM_ADMIN_PASSWORD || "Admin@123456"}`);
        console.log(`   Org Admin: ${orgAdmin.email} / ${process.env.DEMO_ORG_ADMIN_PASSWORD || "OrgAdmin@123"}`);
        console.log(`   Org Member: ${orgMember.email} / ${process.env.DEMO_ORG_MEMBER_PASSWORD || "Member@123"}\n`);
